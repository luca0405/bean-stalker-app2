...
- **Face ID Authentication Temporarily Disabled (January 18, 2025)**: Disabled Face ID/biometric authentication feature to ensure app stability for App Store submission. The feature was causing potential crashes and incorrectly showing for new users. Face ID button and setup options are now hidden using conditional rendering (`{false &&`) until further testing can be completed. This ensures a stable login experience with username/password only.